...
Events::Events(string mobType) {/*21 total*/
	...
	mobs.push_back("A Greedy Corporate Executive");
	...
	for (int i = 0; i < mobs.size(); i++) {
		if (mobs[i] == mobType) {
			...
			if (i == 3 || i == 10 || i == 17) {
				//Stats for Orc/Werewolf/Greedy Corporate Executive
			}
			if (i == 4 || i == 11 || i == 18) {
...
int Events::setCharLevel(int value) {
	for (int i = 0; i < mobs.size(); i++) {
		mobs[i].setCharLevel(value);
	}
}